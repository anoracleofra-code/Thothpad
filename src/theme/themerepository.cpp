...
public:
    ...
    void loadKanagawaLotusTheme();
    void loadKanagawaLotusPaperTheme();
    void loadKanagawaWaveTheme();
    void loadThothPadTealTheme();
    void loadThothPadNeutralTheme();
    ...
    : d_ptr(new ThemeRepositoryPrivate())
{
    ...
    if (!d->themeDirectory.exists()) {
        if (!d->themeDirectory.mkpath(d->themeDirectory.path())) {
            qWarning() << "Could not create theme directory:" << d->themeDirectory.path();
        }
    }
    ...
    d->loadKanagawaLotusTheme();
    d->loadKanagawaLotusPaperTheme();
    d->loadKanagawaWaveTheme();
    d->loadThothPadTealTheme();
    d->loadThothPadNeutralTheme();
}
...
namespace
{
ColorScheme makeColorScheme(const char *background,
                            ...
                            const char *error)
{
    ...
    colors.link = link;
    colors.headingText = heading;
    colors.headingMarkup = markup;
    colors.emphasisText = emphasis;
    colors.emphasisMarkup = markup;
    colors.blockquoteText = block;
    colors.blockquoteMarkup = markup;
    colors.codeText = block;
    colors.codeMarkup = markup;
    colors.image = link;
    colors.listMarkup = accent;
    colors.inlineHtml = markup;
    colors.divider = markup;
    colors.error = error;
    return colors;
}
}
...
void ThemeRepositoryPrivate::loadKanagawaLotusPaperTheme()
{
    const ColorScheme colors =
        makeColorScheme("#f4eed1", "#222222", "#c8bb86", "#43436c", "#4d699b", "#111111", "#624c83", "#222222", "#8a8980", "#716e61", "#c84053");
    builtInThemes.append(Theme("Kanagawa Lotus Paper", colors, colors, true));
}

void ThemeRepositoryPrivate::loadKanagawaWaveTheme()
{
    const ColorScheme colors =
        makeColorScheme("#1f1f28", "#dcd7ba", "#2d4f67", "#dcd7ba", "#7e9cd8", "#e6c384", "#957fb8", "#98bb6c", "#727169", "#54546d", "#e82424");
    builtInThemes.append(Theme("Kanagawa Wave", colors, colors, true));
}

void ThemeRepositoryPrivate::loadThothPadTealTheme()
{
    const ColorScheme colors =
        makeColorScheme("#071f1e", "#ccfbf1", "#115e59", "#2dd4bf", "#5eead4", "#f0fdfa", "#99f6e4", "#ccfbf1", "#5eead4", "#134e4a", "#fb7185");
    builtInThemes.append(Theme("ThothPad Teal", colors, colors, true));
}

void ThemeRepositoryPrivate::loadThothPadNeutralTheme()
{
    const ColorScheme light =
        makeColorScheme("#eff0f1", "#2d363a", "#93cee9", "#3daee9", "#3daee9", "#31363b", "#31363b", "#31363b", "#bdc3c7", "#bdc3c7", "#da4453");
    const ColorScheme dark =
        makeColorScheme("#151719", "#bdc3c7", "#074051", "#3daee9", "#3daee9", "#bdc3c7", "#bdc3c7", "#bdc3c7", "#575b5f", "#575b5f", "#da4453");
    builtInThemes.append(Theme("ThothPad Neutral", light, dark, true));
}
