    ...
    : d(new ChromeColorsPrivate)
{
    ...
    if (thothPadParchment) {
        ...
        d->colors[TertiaryFillColor][NormalState] = QColor("#ebe0b6");
        ...
        d->colors[SecondaryBackgroundColor][NormalState] = lightMode ? mix(base.background, base.selection, 88) : mix(base.background, base.selection, 75);
        d->colors[TertiaryFillColor][NormalState] = lightMode ? mix(base.background, base.selection, 72) : mix(base.background, base.selection, 60);
    }
    d->colors[SecondaryFillColor][NormalState] = base.selection;
    if (thothPadParchment) {
        d->colors[PanelFillColor][NormalState] = QColor("#eae1b9");
        d->colors[ObservationFillColor][NormalState] = QColor("#e6dbad");
        d->colors[SuggestionFillColor][NormalState] = QColor("#dfd5a8");
    } else if (thothPadZinc) {
        ...
        d->colors[PanelFillColor][NormalState] = d->colors[TertiaryFillColor][NormalState];
        d->colors[ObservationFillColor][NormalState] =
            lightMode ? mix(d->colors[TertiaryFillColor][NormalState], base.selection, 72) : mix(d->colors[TertiaryFillColor][NormalState], base.selection, 60);
        d->colors[SuggestionFillColor][NormalState] = d->colors[SecondaryFillColor][NormalState];
    }

    // Commands need a stable, high-contrast surface in every writing theme.
    // The foreground remains theme-derived instead of baking a light-theme
    // color into individual widgets.
    d->colors[StrongActionColor][NormalState] = lightMode ? mix(base.foreground, base.background, 88) : mix(QColor(Qt::black), base.background, 85);
    d->colors[StrongActionTextColor][NormalState] = lightMode ? base.background : base.foreground;
    ...
    // Preserve the redesign's authored, low-contrast panel borders. The
    // generic contrast mix is intentionally reserved for legacy themes.
    if (thothPadParchment) {
        d->colors[SecondarySeparatorColor][NormalState] = QColor("#d4c894");
        d->colors[GridColor][NormalState] = QColor("#d4c894");
    } else if (thothPadZinc) {
