...
static QString writeTimeText(int minutes) { return StatisticsIndicator::tr("%1:%2 write time")
                                                        .arg((int) (minutes / 60), 2, 10, QChar('0'))
                                                        .arg((int) (minutes % 60), 2, 10, QChar('0')); }
static QString dialogueRatioText(int percent)
{
    return StatisticsIndicator::tr("%1% dialogue").arg(percent, 2, 10, QChar('0'));
}
    ...
    : QComboBox(parent)
{
    ...
    this->addItem(dialogueRatioText(0));
    this->setItemData(index, Qt::AlignCenter, Qt::TextAlignmentRole);
}
...
void StatisticsIndicator::setDialogueRatio(int percent)
{
    const int index = this->count() - 1;

    if (index < 0) {
        return;
    }

    this->setItemText(index, dialogueRatioText(percent));
