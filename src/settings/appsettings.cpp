    ...
    : d_ptr(new AppSettingsPrivate())
{
    ...
    preferredMonospaceFonts << "Cascadia Mono SemiBold"
                            << "Cascadia Mono"
                            << "DejaVu Sans Mono"
                            << "Consolas"
                            << "Lucida Console";
    ...
    const QString preferredMonospaceFont = d->firstAvailableFont(preferredMonospaceFonts);
    QString monospaceFont = QFont(preferredMonospaceFont, 13).toString();
    ...
    // Apply the ThothPad writing baseline once. Later user choices remain
    // authoritative because this migration is versioned and never reruns.
    if (appSettings.value(constants::THOTHPAD_DEFAULTS_VERSION_KEY, 0).toInt() < constants::THOTHPAD_DEFAULTS_VERSION) {
        appSettings.setValue(constants::GW_THEME_KEY, QStringLiteral("Kanagawa Lotus"));
        appSettings.setValue(constants::GW_DARK_MODE_KEY, false);
        appSettings.setValue(constants::GW_EDITOR_FONT_KEY, QFont(preferredMonospaceFont, 13).toString());
        appSettings.setValue(constants::GW_EDITOR_WIDTH_KEY, EditorWidthWide);
        appSettings.setValue(constants::GW_INTERFACE_STYLE_KEY, InterfaceStyleSquare);
        appSettings.setValue(constants::GW_FOCUS_MODE_KEY, FocusModeParagraph);
        appSettings.setValue(constants::GW_SIDEBAR_OPEN_KEY, true);
        appSettings.setValue(constants::GW_HTML_PREVIEW_OPEN_KEY, false);
        appSettings.setValue(constants::THOTHPAD_DEFAULTS_VERSION_KEY, constants::THOTHPAD_DEFAULTS_VERSION);
        appSettings.sync();
    }
    ...
    d->themeName = appSettings.value(constants::GW_THEME_KEY, QVariant("Kanagawa Lotus")).toString();
    d->darkModeEnabled = appSettings.value(constants::GW_DARK_MODE_KEY, QVariant(false)).toBool();
    ...
    d->editorWidth = (EditorWidth)appSettings.value(constants::GW_EDITOR_WIDTH_KEY, QVariant(EditorWidthWide)).toInt();
    d->interfaceStyle = (InterfaceStyle)appSettings.value(constants::GW_INTERFACE_STYLE_KEY, QVariant(InterfaceStyleSquare)).toInt();
    ...
    d->htmlPreviewVisible = appSettings.value(constants::GW_HTML_PREVIEW_OPEN_KEY, QVariant(false)).toBool();
