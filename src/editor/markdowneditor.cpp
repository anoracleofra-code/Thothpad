...
constexpr int MarkdownMirrorChunkSize = 16 * 1024;
constexpr int RehighlightBatchBudgetMs = 4;

struct MarkdownLineEdit {
    int line = 0;
    int removedLines = 0;
    int addedLines = 0;
};

class MarkdownTextMirror
{
public:
    struct Snapshot {
    ...
    void setText(const QString &text)
    {
        m_chunks.clear();
        m_length = text.size();
        appendChunks(m_chunks, text);
    }

    bool applyChange(qsizetype position, qsizetype removed, const QString &replacement)
    {
        if ((position < 0) || (removed < 0) || (position > m_length) || (position + removed > m_length)) {
            return false;
        }

        if (m_chunks.isEmpty()) {
            m_length = replacement.size();
            appendChunks(m_chunks, replacement);
            return true;
        }

        const Location start = locate(position);
        const Location end = locate(position + removed);
        QVector<QString> updated;
        updated.reserve(m_chunks.size() + (replacement.size() / MarkdownMirrorChunkSize) + 2);

        for (int index = 0; index < start.chunk; ++index) {
            updated.append(m_chunks.at(index));
        }

        QString changed;
        changed.reserve(start.offset + replacement.size() + m_chunks.at(end.chunk).size() - end.offset);
        changed.append(m_chunks.at(start.chunk).left(start.offset));
        changed.append(replacement);
        changed.append(m_chunks.at(end.chunk).mid(end.offset));
        appendChunks(updated, changed);

        for (int index = end.chunk + 1; index < m_chunks.size(); ++index) {
            updated.append(m_chunks.at(index));
        }

        m_chunks.swap(updated);
        m_length += replacement.size() - removed;
        return true;
    }

    int countNewlines(qsizetype position, qsizetype length) const
    {
        if ((length <= 0) || (position < 0) || (position >= m_length)) {
            return 0;
        }

        const qsizetype end = qMin(m_length, position + length);
        qsizetype chunkStart = 0;
        int count = 0;

        for (const QString &chunk : m_chunks) {
            const qsizetype chunkEnd = chunkStart + chunk.size();
            if (chunkEnd > position && chunkStart < end) {
                const qsizetype localStart = qMax<qsizetype>(0, position - chunkStart);
                const qsizetype localEnd = qMin<qsizetype>(chunk.size(), end - chunkStart);
                count += chunk.mid(localStart, localEnd - localStart).count(QLatin1Char('\n'));
            }
            if (chunkEnd >= end) {
                break;
            }
...
private:
    ...
    static void appendChunks(QVector<QString> &destination, const QString &text)
    {
        for (qsizetype offset = 0; offset < text.size(); offset += MarkdownMirrorChunkSize) {
            destination.append(text.mid(offset, MarkdownMirrorChunkSize));
        }
    }

    Location locate(qsizetype position) const
    {
        qsizetype chunkStart = 0;
        for (int index = 0; index < m_chunks.size(); ++index) {
            const qsizetype chunkEnd = chunkStart + m_chunks.at(index).size();
            if (position <= chunkEnd) {
                return {index, position - chunkStart};
            }
            chunkStart = chunkEnd;
        }

        return {static_cast<int>(m_chunks.size() - 1), m_chunks.constLast().size()};
    }

    QVector<QString> m_chunks;
    qsizetype m_length = 0;
};

struct MarkdownParseResult {
    quint64 revision = 0;
    QSharedPointer<MarkdownAST> ast;
    QVector<quint64> signatures;
...
QVector<quint64> alignSignatures(QVector<quint64> signatures, const QVector<MarkdownLineEdit> &edits)
{
    for (const MarkdownLineEdit &edit : edits) {
        if (signatures.size() <= 1) {
            break;
        }

        const int line = qBound(1, edit.line + 1, signatures.size() - 1);
        signatures[line] = 0;
        if (line > 1) {
            signatures[line - 1] = 0;
        }

        const int firstFollowingLine = qMin(line + 1, signatures.size());
        const int removable = qMin(edit.removedLines, signatures.size() - firstFollowingLine);
        if (removable > 0) {
            signatures.remove(firstFollowingLine, removable);
        }
        if (edit.addedLines > 0) {
            signatures.insert(firstFollowingLine, edit.addedLines, 0);
        }

        if (firstFollowingLine < signatures.size()) {
            signatures[firstFollowingLine] = 0;
        }
    }
...
MarkdownParseResult
parseMarkdownSnapshot(MarkdownTextMirror::Snapshot snapshot, quint64 revision, QVector<quint64> installedSignatures, QVector<MarkdownLineEdit> edits)
{
    QString text;
    text.reserve(snapshot.length);
    for (const QString &chunk : std::as_const(snapshot.chunks)) {
        text.append(chunk);
    }

    MarkdownParseResult result;
    result.revision = revision;
    // CmarkGfmAPI owns the process-wide cmark arena and serializes parse and
    // render calls with its internal mutex, so this worker never accesses the
    // non-reentrant arena concurrently with preview/export work.
    result.ast.reset(CmarkGfmAPI::instance()->parse(text, false));
    result.signatures = result.ast->lineSignatures();
    result.signatures.resize(text.count(QLatin1Char('\n')) + 2);

    const QVector<quint64> aligned = alignSignatures(std::move(installedSignatures), edits);
    const int lineCount = result.signatures.size() - 1;
    result.changedBlocks.reserve(lineCount);

    for (int line = 1; line <= lineCount; ++line) {
        if ((line >= aligned.size()) || (aligned.at(line) != result.signatures.at(line))) {
            result.changedBlocks.append(line - 1);
        }
    }
...
public:
    ...
    FocusMode preReaderFocusMode = FocusModeDisabled;
    QBrush fadeColor;
    QColor blockColor;
    ...
    QTimer *markdownParseTimer;
    QTimer *markdownRehighlightTimer;
    QFutureWatcher<MarkdownParseResult> *markdownParseWatcher;
    MarkdownTextMirror markdownTextMirror;
    QVector<MarkdownLineEdit> pendingMarkdownEdits;
    QVector<quint64> installedLineSignatures;
    QVector<int> pendingRehighlightBlocks;
    int nextRehighlightBlock;
    quint64 documentRevision;
    quint64 parsedRevision;
    quint64 rehighlightRevision;
    bool markdownParseRunning;
    bool markdownParseQueued;

    void toggleCursorBlink();
    void parseDocument();
    void finishMarkdownParse();
    void queueRehighlight(const QVector<int> &blocks, quint64 revision);
    ...
    , d_ptr(new MarkdownEditorPrivate(this))
{
    ...
    d->documentRevision = 0;
    d->parsedRevision = 0;
    d->rehighlightRevision = 0;
    d->markdownParseRunning = false;
    d->markdownParseQueued = false;
    d->nextRehighlightBlock = 0;

    this->setDocument(textDocument);
    d->markdownTextMirror.setText(textDocument->toPlainText());
    ...
    d->overlayController = new TextFormatOverlayController(d->textDocument, this);
    d->overlayController->installToolTips(this);
    connect(d->overlayController, &TextFormatOverlayController::overlaysChanged, viewport(), QOverload<>::of(&QWidget::update));
    d->highlighter = new MarkdownHighlighter(this, colors);

    d->markdownParseTimer = new QTimer(this);
    d->markdownParseTimer->setInterval(200);
    d->markdownParseTimer->setSingleShot(true);
    connect(d->markdownParseTimer, &QTimer::timeout, this, &MarkdownEditor::ensureDocumentParsed);

    d->markdownRehighlightTimer = new QTimer(this);
    d->markdownRehighlightTimer->setSingleShot(true);
    connect(d->markdownRehighlightTimer, &QTimer::timeout, this, [d]() {
        d->processRehighlightBatch();
    });

    d->markdownParseWatcher = new QFutureWatcher<MarkdownParseResult>(this);
    connect(d->markdownParseWatcher, &QFutureWatcher<MarkdownParseResult>::finished, this, [d]() {
        d->finishMarkdownParse();
    });
...
MarkdownEditor::~MarkdownEditor()
{
    ...
    d->markdownParseWatcher->setParent(nullptr);
    connect(d->markdownParseWatcher, &QFutureWatcher<MarkdownParseResult>::finished, d->markdownParseWatcher, &QObject::deleteLater);
}
...
TextFormatOverlayController *MarkdownEditor::textFormatOverlayController() const
{
    Q_D(const MarkdownEditor);
    return d->overlayController;
}
...
void MarkdownEditor::paintEvent(QPaintEvent *event)
{
    ...
    // Paint prose and spelling overlays behind glyphs. This keeps the
    // document layout, syntax formats, selections, and undo stack untouched.
    block = firstVisibleBlock();
    offset = contentOffset();
    done = false;
    while (block.isValid() && !done) {
        const QRectF r = blockBoundingRect(block).translated(offset);
        d->overlayController->paintBlock(painter, block, r);
        offset.ry() += r.height();
        done = offset.y() > viewportRect.height();
        block = block.next();
    }
...
void MarkdownEditor::setPlainText(const QString &text)
{
    ...
    d->markdownParseTimer->stop();
    d->overlayController->resetForDocumentReplacement();
    ++d->documentRevision;
    d->parsedRevision = 0;
    d->pendingMarkdownEdits.clear();
    d->installedLineSignatures.clear();
    d->pendingRehighlightBlocks.clear();
    d->markdownRehighlightTimer->stop();
    d->loadingDocument = true;
    d->typingHasPaused = true;
    d->scaledTypingHasPaused = true;
    QPlainTextEdit::setPlainText(text);
    d->loadingDocument = false;
    d->markdownTextMirror.setText(normalizedDocumentText(text));
    d->parseDocument();
}
...
void MarkdownEditor::ensureDocumentParsed()
{
    ...
    if (d->parsedRevision == d->documentRevision) {
        d->markdownParseTimer->stop();
        return;
    }
...
QFuture<QString> MarkdownEditor::textSnapshot() const
{
    Q_D(const MarkdownEditor);
    const MarkdownTextMirror::Snapshot snapshot = d->markdownTextMirror.snapshot();
    return QtConcurrent::run([snapshot]() {
        QString text;
        text.reserve(snapshot.length);
        for (const QString &chunk : snapshot.chunks) {
            text.append(chunk);
        }
        return text;
    });
...
void MarkdownEditor::setReaderMode(bool enabled)
{
    Q_D(MarkdownEditor);

    if (enabled == d->readerMode) {
        return;
    }
    d->readerMode = enabled;

    if (enabled) {
        d->preReaderFocusMode = d->focusMode;
        setFocusMode(FocusModeTypewriter);
        // Detach the highlighter for a plain-prose read; reattach on exit.
        if (nullptr != d->highlighter) {
            d->highlighter->setDocument(nullptr);
        }
    } else {
        setFocusMode(d->preReaderFocusMode);
        if (nullptr != d->highlighter) {
...
bool MarkdownEditor::readerMode() const
{
    Q_D(const MarkdownEditor);
    return d->readerMode;
}
...
void MarkdownEditor::onContentsChanged(int position, int charsRemoved, int charsAdded)
{
    ...
    QTextCursor addedTextCursor(d->textDocument);
    addedTextCursor.setPosition(position);
    addedTextCursor.setPosition(position + charsAdded, QTextCursor::KeepAnchor);
    const QString addedText = normalizedDocumentText(addedTextCursor.selectedText());
    const int removedLines = d->markdownTextMirror.countNewlines(position, charsRemoved);

    if (!d->markdownTextMirror.applyChange(position, charsRemoved, addedText)) {
        qWarning() << "Markdown text mirror rejected a document change" << position << charsRemoved << charsAdded;
        d->markdownTextMirror.setText(d->textDocument->toPlainText());
        d->pendingMarkdownEdits.clear();
        d->installedLineSignatures.clear();
    } else {
        const QTextBlock changedBlock = d->textDocument->findBlock(position);
        d->pendingMarkdownEdits.append(
            {changedBlock.isValid() ? changedBlock.blockNumber() : 0, removedLines, static_cast<int>(addedText.count(QLatin1Char('\n')))});
    }

    ++d->documentRevision;
    const qsizetype documentLength = d->markdownTextMirror.length();
    const int parseDelayMs = documentLength >= 2'000'000 ? 1500 : documentLength >= 500'000 ? 750 : 250;
    d->markdownParseTimer->start(parseDelayMs);
...
{
    if (markdownParseRunning) {
        markdownParseQueued = true;
        return;
    }

    markdownParseRunning = true;
    markdownParseQueued = false;
    const quint64 revision = documentRevision;
    const MarkdownTextMirror::Snapshot snapshot = markdownTextMirror.snapshot();
    const QVector<quint64> signatures = installedLineSignatures;
    const QVector<MarkdownLineEdit> edits = pendingMarkdownEdits;

    markdownParseWatcher->setFuture(QtConcurrent::run([snapshot, revision, signatures, edits]() mutable {
        return parseMarkdownSnapshot(std::move(snapshot), revision, std::move(signatures), std::move(edits));
    }));
}

void MarkdownEditorPrivate::finishMarkdownParse()
{
    Q_Q(MarkdownEditor);

    const MarkdownParseResult result = markdownParseWatcher->result();
    markdownParseRunning = false;

    if (result.revision == documentRevision) {
        ((MarkdownDocument *)q->document())->setMarkdownAST(result.ast);
        installedLineSignatures = result.signatures;
        pendingMarkdownEdits.clear();
        parsedRevision = result.revision;
        markdownParseQueued = false;
        queueRehighlight(result.changedBlocks, result.revision);
        emit q->markdownASTUpdated(result.revision);
    }

    // An edit made this parse stale. Reparse immediately only when an explicit
    // ensure request reached a running worker after the debounce had expired;
    // otherwise the active debounce timer coalesces the remaining edits.
    if (parsedRevision != documentRevision && markdownParseQueued && !markdownParseTimer->isActive()) {
        parseDocument();
    }
}

void MarkdownEditorPrivate::queueRehighlight(const QVector<int> &blocks, quint64 revision)
{
    pendingRehighlightBlocks = blocks;
    nextRehighlightBlock = 0;
    rehighlightRevision = revision;
    markdownRehighlightTimer->stop();

    if (!pendingRehighlightBlocks.isEmpty()) {
        markdownRehighlightTimer->start(0);
    }
}

void MarkdownEditorPrivate::processRehighlightBatch()
{
    Q_Q(MarkdownEditor);

    if (rehighlightRevision != documentRevision) {
        pendingRehighlightBlocks.clear();
        nextRehighlightBlock = 0;
        return;
    }
    ...
    do {
        const int blockNumber = pendingRehighlightBlocks.at(nextRehighlightBlock++);
        const QTextBlock block = q->document()->findBlockByNumber(blockNumber);
        if (block.isValid()) {
            highlighter->rehighlightBlock(block);
        }
    } while ((nextRehighlightBlock < pendingRehighlightBlocks.size()) && (budget.elapsed() < RehighlightBatchBudgetMs));

    if (nextRehighlightBlock < pendingRehighlightBlocks.size()) {
        markdownRehighlightTimer->start(0);
    } else {
        pendingRehighlightBlocks.clear();
        nextRehighlightBlock = 0;
