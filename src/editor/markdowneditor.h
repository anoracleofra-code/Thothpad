...
public:
    ...
    /**
     * Returns the shared controller for transient text layout overlays.
     */
    TextFormatOverlayController *textFormatOverlayController() const;
    ...
    */
    ...
    /**
     * Parses the current document immediately when its Markdown AST is stale.
    ...
    /**
     * Cold-reader mode: typewriter focus with markdown highlighting
     * detached for a plain, distraction-free read. State restores exactly
     * on exit.
     */
    void setReaderMode(bool enabled);
    bool readerMode() const;
...
signals:
    ...
    /** Emitted after a current Markdown AST has been installed. */
    void markdownASTUpdated(quint64 revision);
