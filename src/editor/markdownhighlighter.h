...
signals:
    /**
     * Emitted after this class has calculated syntax formats for a block.
     * Consumers should capture the block synchronously, then defer layout
     * changes because QSyntaxHighlighter applies its formats after
     * highlightBlock() returns.
     */
    void blockHighlightingCompleted(const QTextBlock &block);
