...
void MarkdownHighlighter::highlightBlock(const QString &text)
{
    ...
    emit blockHighlightingCompleted(currentBlock());
}
...
void MarkdownHighlighterPrivate::applyFormattingForNode(const MarkdownNode *const node)
{
    ...
    // Clear out any markup ranges from the block user data.
    TextBlockData *blockData = (TextBlockData *) q->currentBlock().userData();

    if (nullptr == blockData) {
        blockData = new TextBlockData();
        q->currentBlock().setUserData(blockData);
    } else {
        blockData->clearMarkup();
    }
    ...
    while (!nodes.isEmpty()) {
        ...
        if (lineMatchesNode(currentLine, current)) {
            ...
            case MarkdownNode::Emph:
                ...
                blockData->markup.append({pos, pos + length, MarkdownNode::Emph});
                break;
            case MarkdownNode::Strong:
                ...
                format.setForeground(colors.emphasisMarkup);
                format.setFontWeight(QFont::Bold);
                blockData->markup.append({pos, pos + length, MarkdownNode::Strong});
                break;
            ...
            case MarkdownNode::Strikethrough:
                ...
                blockData->markup.append({pos, pos + length, MarkdownNode::Strikethrough});
                break;
            default:
