...
private slots:
    ...
    void writingTabsNavigateEditAndPersist()
    {
        ...
        const QString manuscript = "# Chapter One\n\nMara said, \"Stay here.\"\n\n## At the gate\n\nBezu asked, \"Why here?\"\n\n\"No one knows.\"\n\n# Chapter Two\n\nMara said, \"Come here.\"";
        ...
        controller.start();
        auto mara = StoryWorkspace::agentTemplate("character"); mara.insert("name", "Mara");
        auto bezu = StoryWorkspace::agentTemplate("character"); bezu.insert("name", "Bezu");
        controller.m_workspace.putAgent(mara); controller.m_workspace.putAgent(bezu);
        ...
        const auto clickTab = [prose](const QString &title) {
            for (auto *b : prose->findChildren<QToolButton *>("writingWorkspaceTab")) if (b->text() == title) { b->click(); return; }
        ...
        const auto clickAction = [&workbench, lines](const QString &title) {
            ...
            if (card && (title == "Go to" || title == "Edit line…")) {
                ...
                QVERIFY(b); b->click(); return;
            }
            if (card && title == "Speaker…") { card->findChild<QPushButton *>("dialogueAssign")->click(); return; }
            for (auto *b : workbench.dialoguePage()->findChildren<QPushButton *>()) if (b->text()==title) { b->click(); return; }
        ...
        editor->undo(); QCOMPARE(editor->toPlainText(),manuscript);
        workbench.refresh();
        ...
        lines->setCurrentItem(lines->topLevelItem(0));
        clickAction("Edit line…");
        editor->moveCursor(QTextCursor::End); editor->insertPlainText(" Changed while editing.");
        ...
        QTest::qWait(150);
        QTimer::singleShot(0, [] { auto *dialog=qobject_cast<QInputDialog *>(QApplication::activeModalWidget()); QVERIFY(dialog); dialog->setTextValue("M\nMara Vale"); dialog->accept(); });
        clickAction("Aliases…");
        ...
        StoryWorkspace disk; QString error;
        ...
        workbench.refresh(); QTRY_VERIFY(workbench.current());
        ...
        QTimer::singleShot(0, [] {
            ...
            const auto inputs = dialog->findChildren<QLineEdit *>(); QCOMPARE(inputs.size(),2);
            inputs[0]->setText("here"); inputs[1]->setText("there");
            auto *preview=dialog->findChild<QPlainTextEdit *>(); QVERIFY(preview);
        ...
        QCOMPARE(lines->topLevelItemCount(),1); lines->setCurrentItem(lines->topLevelItem(0));
        QTimer::singleShot(0, [] {
            auto *dialog=qobject_cast<QInputDialog *>(QApplication::activeModalWidget()); QVERIFY(dialog);
            auto *combo=dialog->findChild<QComboBox *>(); QVERIFY(combo); combo->setCurrentText("Bezu"); dialog->accept();
        });
        clickAction("Speaker…");
        workbench.refresh(); QTRY_VERIFY(workbench.current());
        QCOMPARE(lines->topLevelItemCount(),0);
        QCOMPARE(editor->toPlainText(),manuscript);
        speaker->setCurrentIndex(speaker->findData(bezu.value("id").toString())); QCOMPARE(lines->topLevelItemCount(),2);
        ...
        QTRY_VERIFY(workbench.current());
        workbench.saveSnapshot(); workbench.refresh(); QTRY_VERIFY(workbench.current());
        ...
        editor->moveCursor(QTextCursor::End); editor->insertPlainText("\n\nTala said, \"Follow me.\"");
        workbench.refresh(); QTRY_VERIFY(workbench.current());
        const int detected=speaker->findData("detected:tala"); QVERIFY(detected>=0);
        speaker->setCurrentIndex(detected); QCOMPARE(lines->topLevelItemCount(),1);
        workbench.m_saveCharacter->click(); workbench.refresh(); QTRY_VERIFY(workbench.current());
        bool foundTala=false;
        for(const auto &v:controller.m_workspace.data.value("agents").toArray()) if(v.toObject().value("name").toString()=="Tala") foundTala=true;
        QVERIFY(foundTala);
    ...
    void chatActionsPersistAndReplayThroughTheEngine()
    {
        ...
        QTimer::singleShot(0, []() {
            ...
            QVERIFY(dialog); dialog->setTextValue("A revised prompt"); dialog->accept();
        });
        ...
        controller.m_workspace.data.insert("markers", QJsonArray{QJsonObject{{"id", "owned"}, {"session_id", controller.m_sessionId}, {"message_id", answerId}}});
        QTimer::singleShot(0, []() { auto *box = qobject_cast<QMessageBox *>(QApplication::activeModalWidget()); QVERIFY(box); box->button(QMessageBox::No)->click(); });
        widget.messageActionRequested(answerId, "delete");
        QCOMPARE(controller.m_history.size(), 2);
        QTimer::singleShot(0, []() { auto *box = qobject_cast<QMessageBox *>(QApplication::activeModalWidget()); QVERIFY(box); box->button(QMessageBox::Yes)->click(); });
        widget.messageActionRequested(answerId, "delete");
        ...
        for (const auto &v : disk.data.value("sessions").toArray()) {
            ...
            if (s.value("id").toString() == emptySession) { foundEmpty = true; QVERIFY(s.value("messages").toArray().isEmpty()); }
            if (s.value("id").toString() == originalSession) QCOMPARE(s.value("messages").toArray(), originalHistory);
        }
        QVERIFY(foundEmpty);
    ...
    void workspaceWiringPreservesScopesSessionsAndSafeEdits()
    {
        ...
        editor->setPlainText(manuscript);editor->ensureDocumentParsed();
        QTRY_VERIFY(editor->isDocumentParsed());
        QCOMPARE(editor->toPlainText(),manuscript);
        StoryIntelligenceWidget widget(&window);
        widget.hide();
        ...
        CredentialStore credentials;
        AgentEditTransactionManager transactions(editor,window.mainDocumentManager());
        StoryToolHarness harness(&window,editor,window.mainDocumentManager(),window.mainProseController(),
                                 window.mainProseAwarenessWidget(),&transactions);
        StoryIntelligenceController controller(editor,&widget,&engine,&credentials);
        ...
        auto a=StoryWorkspace::agentTemplate("character");a.insert("name","Mara");a.insert("knowledge","Mara private knowledge");
        auto b=StoryWorkspace::agentTemplate("character");b.insert("name","Bezu");b.insert("knowledge","Bezu private knowledge");
        controller.m_workspace.putAgent(a);controller.m_workspace.putAgent(b);
        auto root=controller.m_workspace.scope("manuscript");auto settings=root.value("settings").toObject();
        settings.insert("cast",QJsonArray{a.value("id"),b.value("id")});root.insert("settings",settings);controller.m_workspace.setScope(root);
        controller.refreshWorkspace();
        widget.setActiveCharacter(a.value("id").toString());controller.newSession();
        ...
        QVERIFY(context.contains("Mara private knowledge"));QVERIFY(!context.contains("Bezu private knowledge"));
        widget.setActiveCharacter(b.value("id").toString());controller.newSession();
        QVERIFY(controller.m_history.isEmpty());
        ...
        for(const auto &v:controller.allowedManifest()) QVERIFY(v.toObject().value("risk").toString()!="R3" && v.toObject().value("risk").toString()!="R4");
        controller.m_scopeMode="scene";
        auto cursor=editor->textCursor();cursor.setPosition(manuscript.indexOf("quiet"));editor->setTextCursor(cursor);
        controller.refreshWorkspace();
        QCOMPARE(editor->toPlainText(),manuscript);
        ...
        cursor.setPosition(manuscript.indexOf("Rain"));editor->setTextCursor(cursor);
        controller.refreshWorkspace();
        QVERIFY(controller.m_history.isEmpty());
        QCOMPARE(scopePicker->currentText(), QString("Current chapter"));
        QCOMPARE(widget.findChild<QLabel *>("storyContextHeadingLabel")->text(), QString("Chapter: Two"));
        cursor.setPosition(manuscript.indexOf("lamp"));editor->setTextCursor(cursor);
        controller.refreshWorkspace();
        ...
        QCOMPARE(controller.m_scopeId, chapterScope);
        settings.insert("cast", QJsonArray{b.value("id")});root.insert("settings", settings);controller.m_workspace.setScope(root);
        widget.sessionSelected(privateSession);
        QCOMPARE(controller.m_sessionId, firstChapterSession);
        QCOMPARE(controller.m_history.size(), 1);
        ...
        settings.insert("cast", QJsonArray{a.value("id"), b.value("id")});root.insert("settings", settings);controller.m_workspace.setScope(root);
        widget.scopeModeChanged("manuscript");
        widget.setActiveCharacter(QString());controller.newSession();
        ...
        QJsonObject mark{{"id","test-mark"},{"start_utf16",start},{"end_utf16",start+4},{"quote","lamp"},
            {"comment","Try a different light source."},{"category","rewrite"},{"replacement","candle"},
            {"document_revision",controller.m_revision}};
        controller.finishChatTurn({{"message","Consider the lamp."},{"annotations",QJsonArray{mark}},
        ...
        controller.applySuggestion("test-mark");
        QVERIFY(editor->toPlainText().contains("candle"));editor->undo();QCOMPARE(editor->toPlainText(),manuscript);
        const auto savedSession=controller.m_sessionId;
        const auto path=doc->filePath();
        doc->clear();editor->setPlainText("# Other\n\nAnother book.");doc->setFilePath(dir.filePath("other.md"));
        controller.openWorkspaceForDocument();
        QVERIFY(controller.m_workspace.agent(a.value("id").toString()).isEmpty());
        QVERIFY(controller.m_history.isEmpty());
        doc->clear();editor->setPlainText(manuscript);doc->setFilePath(path);editor->ensureDocumentParsed();
        QTRY_VERIFY(editor->isDocumentParsed());
        controller.openWorkspaceForDocument();
        QCOMPARE(controller.m_sessionId,savedSession);QCOMPARE(controller.m_history.size(),2);
        QVERIFY(!controller.m_workspace.agent(a.value("id").toString()).isEmpty());
        bool retained=false;for(const auto &v:controller.m_workspace.data.value("sessions").toArray())if(v.toObject().value("id").toString()==privateSession)retained=true;
        QVERIFY(retained);
        doc->setFilePath(dir.filePath("copy.md"));controller.openWorkspaceForDocument();QVERIFY(controller.saveWorkspace());
        QCOMPARE(controller.m_history.size(),2);QVERIFY(QFileInfo::exists(dir.filePath(".thothpad/copy.md.story.json")));
        QTimer::singleShot(0,[](){
            auto *dialog=qobject_cast<QDialog*>(QApplication::activeModalWidget());QVERIFY(dialog);
        ...
        editor->document()->setModified(false);QVERIFY(window.close());
    }
    void shellKeepsManuscriptVisibleWhenEitherPaneResizes()
    {
        ...
        auto *editor = window.mainEditor();
        const QString paragraph = QStringLiteral("The wind moved through the trees, carrying the scent of rain. "
        ...
        story->setSessions({QJsonObject{{"id", "first"}, {"label", "Explore the ending · Chapter one · Co-Writer"}},
            QJsonObject{{"id", "second"}, {"label", "Alternate ending · Chapter one · Co-Writer"}}}, "first");
        ...
        for (const auto *name:{"deleteActionButton","backObservationButton","nextObservationButton","undoActionButton","findingActionsButton"}) {
            auto *button=prose->findChild<QToolButton*>(name);QVERIFY(button);
