...
* Pacing heatmap: outline headings tint by section rhythm — amber for dialogue-heavy (fast), blue for dense narration (slow).
* Reader Mode (View → Reader Mode, Ctrl+Alt+R): fullscreen, typewriter focus, and plain unhighlighted prose for cold-read passes; all prior state restores on exit.
* Breath map: a sentence-rhythm strip beside the editor drawing one bar per sentence, flagging monotonous same-length runs.
* Manuscript reports: crutch words now carry per-chapter counts (`per_chapter`) plus chapter names, and the Markdown export prints the per-chapter curve.
* Manuscript reports: chapter-opener audit flags repeated first words (3+ chapters) and repeated opening bigrams (2+ chapters).
* Analysis envelopes now include a `dialogue` block: span count, dialogue word ratio, and tag-verb histogram, identical across live, cancelled-live, and full presets.
* The statistics indicator gained a dialogue-ratio entry fed by every analysis response.
* Manuscript envelopes embed a `quality_timeline` of ordered ledger runs; the Markdown report export renders it as a table, and the `quality_timeline` operation / `prose_quality_timeline` MCP tool query it directly.
* Calibrations store per-analyzer genre baselines (median flags-per-1000-words); manuscript reports embed a `genre_comparison` section rendered in exports, with the `lens_baselines` operation / `prose_lens_baselines` MCP tool for direct access.
* Engine quality floors: ruff lint and mypy type-check enforced, marker-based pytest lanes (`windows`, `requires_harper`, `requires_spacy_model`), single-source `ENGINE_VERSION`, and CI lint/typecheck jobs.
* Opt-in prose instrumentation build (`THOTHPAD_INSTRUMENTATION`) with six performance signals for measurement runs.
...
* Analysis preparation (text hashing, exclusion scans) moved off the GUI thread; markdown exclusion regexes compile once.
* Edits fully inside an excluded span (code blocks, links) grow or shrink the span instead of forcing full-document resynchronization.
* Snapshot refreshes apply overlay deltas per block with viewport-first ordering instead of clearing and rebuilding all prose highlights.
* Spelling re-checks run debounced in bounded ticks; language detection happens once per block rather than per sentence.
* HTML preview exports coalesce behind a 300 ms quiet window and skip re-export when text is unchanged.
* Prose dialogs route through the parent-aware MessageBoxHelper; dead Qt5-era code paths, the unity-build memoryarena double-compile, and unchecked directory creations were removed.
* Project and quality-ledger writes are atomic; corrupt project metadata no longer breaks project listings.
* The MCP server honors the configured default profile instead of a hard-coded one, and provider failures are logged rather than silently swallowed.
