...
class _StoryHTMLParser(HTMLParser):
    ...
    def _append_text(self, value: str) -> None:
        ...
        self._ends_newline = value.endswith("\n")

    def _append_break(self) -> None:
        if self._text_length and not self._ends_newline:
            self._append_text("\n")
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if len(folded) == 2 and folded.startswith("h") and folded[1].isdigit():
            ...
            self._heading_start = self._text_length
            self._heading_text = []
        if folded == "a":
            ...
            if href:
                self._href = href
                self._link_start = self._text_length
                self._link_text = []
...
class HtmlAdapter(SourceAdapter):
    ...
    def extract(self, candidate: SourceCandidate) -> ExtractedDocument:
        ...
        parser.feed(source)
        parser.close()
        # Keep parser offsets in the same coordinate space as the returned text.
        # Stripping newlines here would shift exact provenance/link offsets.
        text = parser.text
        structures = []
